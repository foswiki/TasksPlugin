...
The corresponding checker abstract class is Foswiki::Configure::Checkers::Tasks::ScheduleChecker.
...
Foswiki::Configure::Checkers::CleanupSchedule as a template for creating a specialized  instance.
Name it Foswiki::Configure::Checkers::Plugins::<plugin-name>::<YourSchedule>.pm
...
my $patched = 1;
if( 0 ) { # Would be nice to test for release and skip check.  Can't, but can remove this if integrated into core.
...
---++ ClassMethod new( $id )
Constructor for a new Foswiki::Configure::Types::SCHEDULE object
...
sub prompt {
    ...
    defined $value or
	$value = eval "\$Foswiki::defaultCfg->$id" || "1 15 1-31/4 * * 15";
...
The UI calls this method when no item-specific checker class (Foswiki::Confiugre::Checkers::key[::subkey...]) exists.
...
Returns a Foswiki::Configure::Checker object for this schedule.
...
#    my( $item, $keys ) = @_;
    ...
    require Foswiki::Configure::Checkers::Tasks::ScheduleChecker;
    my $checker = Foswiki::Configure::Checkers::Tasks::ScheduleChecker->new( @_ );
